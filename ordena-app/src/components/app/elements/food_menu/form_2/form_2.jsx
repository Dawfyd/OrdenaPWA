...
function Form_2({alt_food, id_food, opt_food, adds}){
  ...
  <div>
    ...
    <hr style={alt_food.filter(b => b.id_alt === id_food).length === 0 ? {display:'none'}:
                                                                         {display:'flex'}}/>
    <div className='alt_opt' style={alt_food.filter(b => b.id_alt === id_food).length === 0 ? {display:'none'}:
                                                                                              {display:'flex'}}>
    ...
    <hr style={alt_food.filter(b => b.id_alt === id_food).length === 0 ? {display:'none'}:
                                                                         {display:'flex'}}/>
    {
    alt_food.filter(b => b.id_alt === id_food)
      .map(j => (
