...
function Form_3({alt_food, id_food, opt_food, adds}){
  return(
  <div>
    <hr style={adds.filter(b => b.id_add === id_food).length === 0 ? {display:'none'}:
                                                                       {display:'flex'}}/>
      <div className='adds_opt' style={adds.filter(b => b.id_add === id_food).length === 0 ? {display:'none'}:
                                                                                             {display:'flex'}}>
        Adicionales
      </div>
    <hr style={adds.filter(b => b.id_add === id_food).length === 0 ? {display:'none'}:
