...
function Form_1({alt_food, id_food, opt_food, adds}){
  return(
  <div>
    <div className='alt_opt' style={opt_food.filter(b => b.id_add === id_food).length === 0 ? {display:'none'}:
                                                                                           {display:'flex'}  }>
      Opciones
    </div>
    <hr style={opt_food.filter(b => b.id_add === id_food).length === 0 ? {display:'none'}:
                                                                         {display:'flex'}}/>
    {
    opt_food.filter(b => b.id_opt === id_food)
      .map(j => (
